...
def title_block(fig, headline, subtitle=None):
    """Headline + optional subtitle, right-aligned in the top corner."""
    fig.suptitle(headline, x=0.985, ha="right", fontsize=15,
                 fontweight="bold", color=COLOR_TEXT)
    if subtitle:
        fig.text(0.985, 0.948, subtitle, ha="right", fontsize=10,
...
def chart_01_who_exceedance(engine):
    ...
    title_block(fig,
                ...
                "Only Da Lat stays mostly clean")
...
def chart_02_monthly_series(engine):
    ...
    ax.legend(ncol=3, fontsize=9.5, loc="upper left",
              bbox_to_anchor=(0.0, 1.16), columnspacing=1.6)

    title_block(fig,
                "April spikes repeat in both years",
                "A pattern, not a one-off. Hanoi never drops below 39 ug/m3")
...
def chart_03_rain_effect(engine):
    ...
    title_block(fig,
                ...
                "Both panels decline monotonically - real, just not linear")
...
def chart_04_weather_correlation(engine):
    ...
    title_block(fig,
                ...
                "Negative = the factor reduces PM2.5")
...
def chart_05_pollution_fingerprint(engine):
    ...
    # Min-max scale each pollutant independently.
    # Do NOT divide by PM2.5 - cities with a small denominator (Da Lat,
    # 10.4) get wildly inflated ratios. Verified and rejected.
    scaled = df.apply(lambda c: 100 * (c - c.min()) / (c.max() - c.min()))
    ...
    ax.set_ylim(0, 128)
    ax.legend(ncol=4, fontsize=10, loc="upper left",
              bbox_to_anchor=(0.0, 1.13), columnspacing=2)
    ...
    title_block(fig,
                ...
                "Same problem, two causes - and two different solutions")
...
def chart_06_month_heatmap(engine):
    ...
    title_block(fig,
                ...
                "HCMC runs in antiphase: April is its cleanest month (1.7%)")
...
def chart_07_daily_rhythm(engine):
    ...
    title_block(fig,
                ...
                "Overnight inversion traps particles; midday mixing clears them")
...
def chart_08_episodes(engine):
    ...
    title_block(fig,
                ...
                "Can Tho holds the longest single run: 42 straight days")
